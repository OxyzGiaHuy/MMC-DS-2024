...
def thong_ke_user(username):
    try:
        du_lieu_choi_lo = read_file(PATH_DATA_CHOI_LO, "r")
        so_lan_choi = 0
        tong_tien_thang = 0
        tong_tien_thua = 0
        so_lan_thang = 0
        ...
        for choi_lo in du_lieu_choi_lo:
            if choi_lo[1] == username:
                ...
                if int(choi_lo[-2]) >  0:
                    ...
                    so_lan_thua += 1
        ti_le_thang = round(so_lan_thang / so_lan_choi, 2)
...
def thong_ke_admin():
    try:
        ...
        du_lieu_choi_lo = read_file(PATH_DATA_CHOI_LO, "r")
        so_luong_tai_khoan = len(read_file(PATH_DATA_TAI_KHOAN, "r"))
        ...
        ti_le_thang_thua = round(so_lan_thang / (tong_so_lan_choi_lo), 2)
